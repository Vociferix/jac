...
class option_impl {
  ...
  public:
    ...
    constexpr void swap(option_impl& other) {
...
class option_impl<T&> {
  ...
  public:
    ...
    template <typename U>
    explicit constexpr option_impl(U* ptr)
        : value_(static_cast<T*>(ptr)) {}

    template <typename U>
    explicit constexpr option_impl([[maybe_unused]] std::in_place_t in_place, U& arg)
...
class option_impl<void_t> {
  ...
  public:
    ...
    using const_rvalue_reference = typename holder<void_t>::const_rvalue_reference;
    using pointer = typename holder<void_t>::pointer;
    using const_pointer = typename holder<void_t>::const_pointer;

    template <typename... Args>
    explicit constexpr option_impl([[maybe_unused]] std::in_place_t in_place, [[maybe_unused]] Args&&... args)
        : has_value_(true) {}
...
class option_impl<void> : public option_impl<void_t> {
  public:
    ...
    using const_rvalue_reference = typename option_impl<void_t>::const_rvalue_reference;
...
///
/// @details
/// `option` is modeled after `std::optional`, but `option` supports references and `void`.
/// When the wrapped type is a reference, `option` functions as a lightweight smart pointer
/// and interoperates with raw pointers. When the wrapped type is `void`, `jac::void_t` is
/// used as the value type, and `option` essentially functions as a `bool`.
template <typename T>
class option {
  private:
  ...
  public:
    ...
    constexpr option([[maybe_unused]] null_t null)
        : option() {}
    ...
    template <typename... Args>
    constexpr explicit(sizeof...(Args) == 0) option(std::in_place_t in_place, Args&&... args)
        : impl_(in_place, std::forward<Args>(args)...) {}

    template <typename U, typename... Args>
    constexpr option(std::in_place_t in_place, std::initializer_list<U> ilist, Args&&... args)
        : impl_(in_place, ilist, std::forward<Args>(args)...) {}

    template <typename U>
        requires(
            std::is_constructible_v<value_type, typename option<U>::const_reference> &&
    ...
    constexpr explicit(!std::is_convertible_v<typename option<U>::const_reference, value_type>) option(const option<U>& other) {
        if (other.has_value()) {
            impl_.emplace(*other);
        }
    }
    ...
    template <typename U>
        requires(
            std::is_constructible_v<value_type, typename option<U>::rvalue_reference> &&
            !std::is_constructible_v<value_type, option<U>&> &&
            !std::is_constructible_v<value_type, const option<U>&> &&
            !std::is_constructible_v<value_type, option<U>&&> &&
            !std::is_constructible_v<value_type, const option<U>&&> &&
    ...
    constexpr explicit(!std::is_convertible_v<typename option<U>::rvalue_reference, value_type>) option(option<U>&& other) {
        if (other.has_value()) {
    ...
    constexpr explicit(!std::is_convertible_v<U&&, value_type>) option(U&& value)
        : impl_(std::in_place, std::forward<U>(value)) {}

    template <typename U = std::remove_reference_t<T>>
        requires(std::is_lvalue_reference_v<U>)
    constexpr explicit(!std::is_convertible_v<U*, T*>) option(U* ptr) : impl_(ptr) {}
    ...
    constexpr auto transform(F&& f) && {
        ...
        if (impl_.has_value()) {
            return option<ret_t>(std::invoke(std::forward<F>(f), std::move(impl_).get()));
        } else {
            return opiont<ret_t>();
        }
    }
...
template <typename T>
constexpr std::strong_ordering operator<=>(const option<T>& lhs, [[maybe_unused]] null_t rhs) {
    return lhs.has_value() <=> false;
}

template <typename T>
constexpr std::strong_ordering operator<=>([[maybe_unused]] null_t lhs, const option<T>& rhs) {
...
template <typename T>
constexpr std::strong_ordering operator<=>([[maybe_unused]] std::nullopt_t lhs, const option<T>& rhs) {
    return false <=> rhs.has_value();
}
...
template <typename T>
    requires(std::is_lvalue_reference_v<T>)
constexpr std::strong_ordering operator<=>([[maybe_unused]] std::nullptr_t lhs, const option<T>& rhs) {
    return false <=> rhs.has_value();
}
...
constexpr std::compare_three_way_result_t<U, T> operator<=>(const T& lhs, const option<U>& rhs) {
    if (rhs.has_value()) {
        return lhs <=> *rhs;
    } else {
        return true <=> false;
...
template <typename T, typename U>
    requires(std::is_lvalue_reference_v<T>)
constexpr std::compare_three_way_result_t<const T*, U> operator<=>(const option<T>& lhs, const U* rhs) {
    return lhs.operator->() <=> rhs;
}

template <typename T, typename U>
    requires(std::is_lvalue_reference_v<U>)
constexpr std::compare_three_way_result_t<T, const U*> operator<=>(const T* lhs, const option<U>& rhs) {
...
struct std::hash<::jac::option<T>> {
  ...
  public:
    constexpr size_t operator()(const ::jac::option<T>& value) const {
        ...
        if (has_val) {
            h = ::jac::hash_combine(h, inner_(value));
        }
        return h;
    }
};
